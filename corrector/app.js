...
function cargar() {
  try {
    ...
    if (dato && Array.isArray(dato.alumnos) && dato.claves && dato.registros) return dato;
    ...
    <div class="tarjeta">
      ...
      <div class="entrada">
        <input type="text" id="in-resp" autocomplete="off" spellcheck="false" autocapitalize="characters"
          placeholder="Teclea las respuestas en orden: ABBCAD…   («-» en blanco, «?» nula)" value="${esc(ui.resp)}">
...
function pintarBloques() {
  ...
  for (let b = 0; b < Math.ceil(n / POR_BLOQUE); b++) {
    let t = '<table class="bloque"><tr><th>Q</th>' + [...L.LETRAS].map(l => `<th>${l}</th>`).join('') + '</tr>';
