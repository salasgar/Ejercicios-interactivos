...
Actualizado: 2026-09-10 · Sesiones previas: 3
...
**Batería de ejercicios — fase 1 de la ampliación, terminada, 89 tests en
verde (`npm test`):** 11 tipos de ejercicio (9 de cálculo de 1º ESO +
...
Incorrecto». Con la fase 1 recién añadida (`raices`,
`producto_division_fracciones`, división en `decimales`), **la 1ª evaluación
de 1º ESO queda completa (UD1, UD2, UD3) y la 2ª también salvo Álgebra
(UD6)**. Ver la hoja de ruta completa más abajo.
...
## Siguiente paso
Seguir la hoja de ruta de la ampliación de ejercicios por la **fase 2**: UD6
Álgebra de 1º ESO (17 sesiones lectivas, 2ª evaluación) — tipos nuevos
`expresiones_algebraicas` (traducir enunciados, valor numérico, términos
semejantes) y `ecuaciones_primer_grado` (ax+b=c, con paréntesis sencillos).
Cierra la 2ª evaluación de 1º ESO. Seguir el mismo patrón que `raices.js` y
`producto_division_fracciones.js` (ver «Archivos»); añadir los ids nuevos a
`TIPOS`/`CONCEPTOS` en `src/ejercicios/index.js` y, si llevan solución paso a
paso, a `CON_PASOS` en `tests/ejercicios.test.js`; `npm test` debe seguir en
verde antes de dar la fase por terminada.
...
- `src/ejercicios/raices.js`, `src/ejercicios/producto_division_fracciones.js` — los dos tipos nuevos de la fase 1; buen ejemplo reciente del patrón para los tipos que faltan.
...
- `tests/ejercicios.test.js` — `CON_PASOS` (tipos con solución paso a paso comprobada), 89 tests (`npm test`).
